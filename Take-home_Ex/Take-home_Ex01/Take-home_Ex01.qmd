...
ins2_df <- analysis_data %>%
  mutate(
    time_studying_val = suppressWarnings(as.numeric(as.character(time_studying))),
    gpa_val = suppressWarnings(as.numeric(as.character(gpa))),
    Time_Studying_cat = factor(time_studying_val, levels = 1:5, labels = study_order, ordered = TRUE),
    GPA_cat = factor(gpa_val, levels = 1:5, labels = gpa_order, ordered = TRUE)
  ) %>%
  filter(!is.na(Time_Studying_cat), !is.na(GPA_cat)) %>%
  count(Time_Studying_cat, GPA_cat) %>%
  group_by(Time_Studying_cat) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup()

ggplot(ins2_df, aes(x = Time_Studying_cat, y = pct, fill = GPA_cat)) +
  geom_col(width = 0.75) +
...
analysis_data %>%
  ...
  count(support_cat, GPA_cat) %>%
  group_by(support_cat) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  ggplot(aes(x = support_cat, y = pct, fill = GPA_cat)) +
  geom_col(width = 0.75) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
...
cor_df <- cor(eda_vars, use = "complete.obs") %>%
  as.data.frame() %>%
...
- Strong positive correlations are observed among perceived learning environment variables, including lecturer quality, teaching support, and curriculum training, suggesting these perceptions tend to co-vary.
- Time spent studying shows moderate positive association with perceived study methods and learning support, indicating that more engaged students may also perceive the learning environment more positively.
- In contrast, time spent with friends exhibits weaker or negative correlations with academic-related perceptions, highlighting a potential trade-off between social time and learning engagement.
- Overall, the results suggest that students’ learning behaviours and perceptions form coherent patterns, providing useful context for subsequent analyses involving academic outcomes.
